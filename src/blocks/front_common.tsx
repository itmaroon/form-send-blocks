...
引数： msg 画面に表示する文言
------------------------------ */
export function dispLoading(msg: string, target: JQuery) {
	// 引数なし（メッセージなし）を許容
	if (msg == undefined) {
		msg = "";
	}
...
Loading イメージ削除関数
------------------------------ */
export function removeLoading(dispMsg: string, target: JQuery) {
	if (target == undefined) {
		//ターゲット指定がないときはbodyにつける
		target = jQuery("body");
	}
...
export const sendMail_ajax = (
	send_email: string | number | string[] | undefined,
	subject_mail: string,
	message_mail: string,
	master_email: string,
	master_name: string,
...
export function evaluateCheckboxes($target_form: JQuery) {
...
//メッセージ再構築関数
export const message_rebuild = (message: string): string => {
...
//必須項目のバリデーションチェック
export function require_check($: JQueryStatic, form: JQuery) {
		...
		.each(function () {
			...
			if (required) {
				...
				const select_flg = select_elm?.attr("multiple")
					...
					: !select_val; // 単数選択時は値の有無をチェック
				let required_err = false;
				if (input_elm) {
					const input_val = input_elm.val();
					// 配列や数値の可能性を考慮して、文字列として評価
