...
export default function Edit({ attributes, setAttributes, context, clientId }) {
	...
	// 紐付けを更新する関数
	const updateMapping = (blockId, tableId) => {
	...
	// セル要素を生成する関数
	const cellObjectsForm = (inputFigureInnerBlocks) => {
	...
	useEffect(() => {
		...
		const existingFormNames = inputFigureBlocks
			.map((block) => block.attributes.form_name)
		...
		const newDataSources = inputFigureBlocks.map((block) => {
		...
		<>
			<InspectorControls group="settings">
				<PanelBody title={__("Step-Table Mapping", "itmar")}>
					{inputFigureBlocks.map((block, index) => {
						// 現在このブロックに紐付いているテーブルIDを探す
						const currentMapping = blockTableMapping.find(
							(m) => m.blockId === block.attributes.form_name,
						);
